...
履歴戦略は `createWebHashHistory`（hash-based）を採用しています。これにより GitHub Pages 上でリロード時の 404 を防止しています。
...
- Brotli圧縮: `.br` ファイルを自動生成 (`vite-plugin-compression`)
- ルートレベルのコード分割: Schedule・Library・NotFound は遅延ロード

### CI/CD

| トリガー      | ワークフロー       | 説明                                       |
| ------------- | ----------------- | ------------------------------------------ |
| push to main  | `app-release.yml` | ビルド → GitHub Pages デプロイ → Docker イメージ |
| Pull Request  | `ci.yml`          | Prettier フォーマットチェック + Vite ビルド     |
| pre-commit    | lefthook          | フォーマットチェック（失敗時 commit 阻止）       |
| pre-push      | lefthook          | AI コードレビュー（Blocker/High で push 阻止）  |
